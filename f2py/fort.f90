subroutine sst_acspo2soda_1rec(sst_ac, sst_so, num_sst_so, idx_lat_so2ac, idx_lon_so2ac)
    implicit none

    real   (kind=4), dimension(18000, 9000), intent(in ) :: sst_ac
    real   (kind=4), dimension(360, 180), intent(out) :: sst_so
    integer(kind=4), dimension(360, 180), intent(out) :: num_sst_so
    integer(kind=4), dimension(2, 180), intent(in) :: idx_lat_so2ac
    integer(kind=4), dimension(2, 360), intent(in) :: idx_lon_so2ac

! f2py instructions, for f90 not to use Cf2py.
!f2py intent(in) sst_ac
!f2py intent(out) sst_so
!f2py intent(out) num_sst_so
!f2py intent(in) idx_lat_so2ac
!f2py intent(in) idx_lon_so2ac
    ...
    do j = 1, 180; do i = 1, 360
        lat_min = idx_lat_so2ac(1, j); lat_max = idx_lat_so2ac(2, j)

        if (i == 180) then
            sst_so    (i, j) = sum  ( sst_ac(1:25, lat_min:lat_max) )
            sst_so    (i, j) = sst_so(i, j) + sum  ( sst_ac(17976:18000, lat_min:lat_max) )
            num_sst_so(i, j) = count( sst_ac(1:25, lat_min:lat_max) > 1.0)
            num_sst_so(i, j) = num_sst_so(i, j) + count( sst_ac(17976:18000, lat_min:lat_max) > 1.0)
        else
            lon_min = idx_lon_so2ac(1, i); lon_max = idx_lon_so2ac(2, i)
            sst_so    (i, j) = sum  ( sst_ac(lon_min:lon_max, lat_min:lat_max) )
            num_sst_so(i, j) = count( sst_ac(lon_min:lon_max, lat_min:lat_max) > 1.0)
        end if
    end do; end do

    return
end subroutine sst_acspo2soda_1rec


subroutine write_sst_noaa2soda_5day_avg(fn_sst, ndat, sst_soda)
    implicit none

  ! character(:),                      intent(in) :: fn_sst  ! compile error
    character(256),                    intent(in) :: fn_sst
